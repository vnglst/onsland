 ...
 */
export function renderCountryPreview(countryKey, svgElement, worldData) {
  if (!worldData) return;
 ...
 * @param {Object} worldData - TopoJSON world data
 */
export function renderCountrySquarePreview(countryKey, svgElement, worldData) {
  if (!worldData) return;
...
/**
 * Create square path for rendering
 * @param {number} size - Size of square
 * @returns {string} SVG path string
 */
export function squarePath(size) {
  const half = size / 2;
  return `M ${-half},${-half} L ${half},${-half} L ${half},${half} L ${-half},${half} Z`;
}

/**
 * Calculate square positions for grid layout
 * @param {number} totalHexagons - Total number of items
 * @returns {Array<{x: number, y: number}>} Array of positions
 */
export function calculateSquarePositions(totalHexagons) {
  const squareSize = 10;
